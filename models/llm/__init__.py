...
def generate(content: str, title: str):
  content = cleanStopStrings(content)

  content_pretokenized = Cogenerator.PretokenizeInput(content)
  topic_content_pretokenized = Cogenerator.PretokenizeInput(f"""Title: {title}
{content}""") if title else content_pretokenized
...
for kv in generate(
    ...
    "Advanced Infrastructure Security Implementation"):
  pass
